...
def downsample_batch(img_batch, resolution):
    return np.stack(
        [np.array(Image.fromarray(img).resize((resolution, resolution), Image.LANCZOS)) for img in img_batch]
        )
    ...
    def fetch_batch(self):
        while self.run:
            ...
            if self.multiscale:
                ...
                for res in [256, 128, 64, 32, 16, 8, 4]:
                    multibatch.append((downsample_batch(numpy_batch, res)/127.5)-1)
