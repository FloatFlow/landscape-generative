...
class MixingRegularization(Layer):
    def __init__(self, n_projections, **kwargs):
        self.n_projections = n_projections
        super(MixingRegularization).__init__(**kwargs)

    def build(self, input_shape):
        super(MixingRegularization, self).build(input_shape)

    def call(self, inputs):
        break_point = np.random.randint(low=0, high=self.n_projections)
        shuffled = tf.random_shuffle(inputs)
        tiled_inputs = K.tile(K.expand_dims(inputs, axis=1), (1, self.n_projections, 1))
        tiled_shuffled = K.tile(K.expand_dims(shuffled, axis=1), (1, self.n_projections, 1))
        outputs = K.concatenate([tiled_inputs[:, :break_point, ...], tiled_shuffled[:, break_point:, ...]], axis=1)
        return outputs

    def compute_output_shape(self, input_shape):
        return (input_shape[0], self.n_projections, input_shape[-1])
